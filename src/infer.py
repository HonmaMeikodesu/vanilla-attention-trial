...
def predict_seq2seq(net, src_sentence, src_vocab, tgt_vocab, num_steps,
    ...
    src_tokens = torch.Tensor(d2l.truncate_pad(src_tokens, num_steps, src_vocab["<pad>"]), device=device).type(torch.int).unsqueeze(dim=0)
    # src_tokens (batch_size=1, num_steps)
    enc_outputs = net.encoder(src_tokens)
    dec_state = net.decoder.init_state(enc_outputs)
    
    tgt_tokens = torch.tensor([tgt_vocab["<bos>"]], dtype=torch.int, device=device).unsqueeze(dim=0)
    ...
    for _ in range(num_steps):
        ...
        Y = torch.argmax(dec_output, dim=2).squeeze(dim=0)
